---
title: Hermes Agent 개인정보 처리방침 · Privacy Policy
---

# Hermes Agent 개인정보 처리방침 · Privacy Policy
...
| 항목 | 목적 |
|---|---|
| 보낸 사람의 이메일 주소, 문의 내용과 첨부파일(이름 등 이용자가 메일에 적은 정보 포함) | 문의 확인과 답변 |

**앱이 기기에 저장하는 정보.** 앱은 기능을 제공하기 위해 아래 정보를 이용자 기기의 앱 전용 저장 공간에 저장하며, 이 정보는 회사에 전송되지 않습니다. 이 가운데 API 토큰과 대시보드 로그인 정보는 접속할 때 이용자가 지정한 서버에도 보냅니다(3절).
...
앱에는 AI 모델이 들어 있지 않으며, 대화는 이용자가 지정한 서버의 AI가 처리합니다. 회사는 이용자가 입력한 글·음성·첨부파일이나 AI가 만든 답변을 받지 않으므로 AI 학습에 쓰지 않습니다. 입력한 정보가 AI 학습에 쓰이는지와 그 거부 방법, 부적절한 답변의 신고는 그 서버의 운영자나 서버가 쓰는 AI 모델의 제공자에게 문의해야 합니다. 대화에 민감정보나 주민등록번호 같은 고유식별정보를 입력하지 않도록 주의해 주십시오.
...
- 문의 메일: 답변을 마치면 바로 삭제합니다. 받은 메일과 첨부파일, 문의 내용이 인용된 보낸 답장 등 회사가 가진 사본을 모두 포함합니다.
...
회사는 개인정보를 제3자에게 제공하지 않습니다. 문의 메일을 보관하는 Gmail에 관해서는 7절과 9절에 적었습니다.
...
| Gmail(Google LLC, 미국) | 문의 메일(이메일 주소, 문의 내용, 첨부파일)과 회사가 보낸 답장 | 이용자가 문의 메일을 보낼 때, 회사가 답장할 때 |
...
- 문의 메일의 국외 보관: Google은 전 세계에 서버를 두고 있어 Gmail에 보관되는 메일이 한국 밖의 서버에서 처리될 수 있습니다([Google 개인정보처리방침](https://policies.google.com/privacy?hl=ko) '데이터 이전'). 구체적인 사항은 다음과 같습니다.
  - 보관되는 항목: 문의 메일의 이메일 주소, 문의 내용, 첨부파일, 그에 대한 회사의 답장
  - 보관 국가와 시기·방법: Google이 서버를 둔 국가들(Google LLC의 소재지는 미국)이며, 이용자가 메일을 보내거나 회사가 답장할 때 인터넷을 통해 Google의 메일 서버로 전송되어 보관됩니다.
  - 보관하는 자: Google LLC(1600 Amphitheatre Parkway, Mountain View, CA 94043, USA), 개인정보 문의는 [Google 개인정보 보호 문의](https://support.google.com/policies?p=privpol_privts&hl=ko)
  - 목적과 기간: 회사가 문의를 받고 답하기 위해서이며, 회사가 삭제할 때까지 보관됩니다(5절·10절).
  - 거부 방법과 효과: 이메일로 문의하지 않는 것으로 국외 보관을 거부할 수 있으며, 이 경우 이메일로는 문의할 수 없습니다.
...
이용자와 법정대리인은 개인정보의 열람, 정정·삭제, 처리정지를 요구할 수 있으며, 위임을 받은 대리인을 통해서도 요구할 수 있습니다. 문의 메일에 관한 요구는 15절의 연락처로 보내 주시면 관계 법령에 따라 처리합니다. 앱이 기기에 저장한 정보는 회사가 보유하지 않으므로, 이용자가 앱 설정에서 직접 고치거나 지울 수 있습니다. 이용자가 지정한 서버에 저장된 정보는 그 서버 운영자에게 요구해야 합니다.
...
앱은 광고 식별자, 분석 도구, 추적 쿠키 같은 광고·분석 목적의 자동 수집 장치를 사용하지 않습니다. 다만 이용자가 지정한 대시보드에 로그인한 상태를 유지하기 위해, 그 대시보드가 발급한 인증 쿠키를 디스크에 영구 저장하지 않고 앱 프로세스의 메모리에 보관하며, 쿠키의 도메인·경로 등 적용 조건에 맞는 요청에 함께 보냅니다. 대시보드를 설정하지 않으면 이 쿠키를 받지 않습니다.
...
개인정보 보호 업무와 관련 문의는 아래에서 처리합니다.

- 담당: 에스제이상사 개인정보 보호 담당
- 연락처: elpissjtrading@gmail.com

### 16. 권익침해 구제 방법

개인정보 침해에 관한 상담이나 구제가 필요하면 아래 기관에 문의할 수 있습니다.
...
| Information | Purpose |
|---|---|
| Your email address, message and attachments, including anything you write such as your name | Reading and answering your inquiry |

**What the app keeps on your device.** To work, the app keeps the following in its private storage on your device. None of it is sent to us. The API token and dashboard sign-in details are also sent to your server when the app connects (see "Information sent to the server you choose").
...
The app contains no AI model; your conversations are processed by the AI on the server you choose. Because we never receive the text, audio or attachments you enter or the answers the AI produces, we do not use them to train AI. Whether your input is used for training, how to opt out, and how to report an inappropriate answer are questions for that server's operator or the provider of the AI model it uses. Please take care not to enter sensitive information or identification numbers in conversations.
...
- Emails you send us: deleted as soon as we have answered, together with every copy we hold, including attachments and our replies that quote your message.
...
We do not sell or share personal information, and we do not use it for advertising or tracking. Emails you send us are kept in Gmail, as described under "Service providers" and "Outside services and international transfers". We do not "sell" or "share" personal information as those terms are defined in the California Consumer Privacy Act.
...
| Gmail (Google LLC, United States) | Emails you send us (your email address, message and attachments) and our replies | When you email us and when we reply |
...
- Storage of emails outside your country: Google maintains servers around the world, so mail kept in Gmail may be processed on servers outside the country where you live (see "Data transfers" in the [Google Privacy Policy](https://policies.google.com/privacy?hl=en)). In detail:
  - What is kept: the email address, message and attachments of emails you send us, and our replies to them
  - Where, when and how: in the countries where Google has servers (Google LLC is based in the United States), sent over the internet to Google's mail servers when you email us or we reply
  - Who keeps it: Google LLC, 1600 Amphitheatre Parkway, Mountain View, CA 94043, USA; privacy questions through [Google privacy contact](https://support.google.com/policies?p=privpol_privts&hl=en)
  - Purpose and period: so that we can receive and answer your inquiry, until we delete it (see "How long we keep information" and "Deleting information")
  - How to decline, and its effect: if you do not want your email stored outside your country, do not email us; you will then not be able to contact us by email.
...
The app uses no advertising identifiers, analytics tools, tracking cookies or other tracking for advertising or analytics. To keep you signed in to the dashboard you configure, it keeps the authentication cookie that dashboard issues in the app process's memory, without writing it permanently to disk, and sends it with requests that match the cookie's domain, path and other conditions. If you do not set up a dashboard, the app receives no such cookie.
...
SJ Trading (ElpisSJ) · elpissjtrading@gmail.com
